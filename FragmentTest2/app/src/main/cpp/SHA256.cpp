...
unsigned char** SHA256::padding(char* input) {
    ...
    for (int intI = 0; intI < intBlock; intI++) {
        //	ブロック個数分のメモリを確保する
        output[intI] =
                (unsigned char*)malloc(sizeof(char) * MESSAGE_BLOCK_SIZE);
        ...
        if (intCopyLength < MESSAGE_BLOCK_SIZE) {
            //	ブロックをクリアする
            memset(output[intI], 1, sizeof(char) * MESSAGE_BLOCK_SIZE);
        }
...
void SHA256::compute(unsigned char** block, unsigned int* H) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        //	2. Initialize the eight working variables, a, b, c, d, e, f, g,
