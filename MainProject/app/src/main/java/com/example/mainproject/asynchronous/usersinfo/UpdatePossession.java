...
public class UpdatePossession implements Runnable {
    ...
    public void execute() {
        ...
        executorService.submit(new UpdatePossession(db, name, _DP, _MONEY, callback, errorCallback));
    }

    //void onPreExecute() {}

    void doInBackground() {
        UsersInfoDao usersInfoDa = db.usersInfoDao();
        BaseStatusFragment BSF = new BaseStatusFragment();

        try {
            usersInfoDa.updatePossessionTask(name, BSF.CastString(_DP), BSF.CastString(_MONEY));
        } catch (Exception e) {
            this.exception = e;
        }
    }
